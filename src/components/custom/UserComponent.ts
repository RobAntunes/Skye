import { TresComponent } from '../core/TresComponent.ts';

export class UserComponent extends TresComponent {
  constructor() {
    super({
      user: {
        id: '',
  ...
  override renderTemplate() {
    ...
    if (this.state.isEditing) {
      userDiv.innerHTML = `
        <h2>Edit User</h2>
        <input id="nameInput" type="text" value="${this.state.user.name}">
        <input id="emailInput" type="email" value="${this.state.user.email}">
      ...
      saveButton?.addEventListener('click', () => {
        const nameInput = userDiv.querySelector('#nameInput') as HTMLInputElement;
        const emailInput = userDiv.querySelector('#emailInput') as HTMLInputElement;
